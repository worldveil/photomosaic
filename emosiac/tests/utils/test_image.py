...
from emosiac.utils.image import divide_image, load_png_image, \
...
def test_divide_image_margins():
  ...
  plt.savefig('emosiac/tests/output/divide_image.png')
...
def test_divide_image_exact():
  ...
  # visualization only
  plt.scatter([b[0] for b in box_starts], [b[1] for b in box_starts], marker='+')
  plt.savefig('emosiac/tests/output/divide_image_exact.png')
...
def test_rgb_conversion():
  path = 'emosiac/tests/input/kiss.png'
